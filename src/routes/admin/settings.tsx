...
function SettingsPage() {
  const claim = useMutation({
    ...
    onSuccess: (ok) => ok ? toast.success("Você agora é administrador — recarregue a página") : toast.info("Já existe um administrador"),
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Se ainda não há nenhum administrador no sistema, qualquer usuário autenticado pode se tornar o primeiro admin.
            </p>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {cards.map((c) => (
          <div key={c.title} className="rounded-2xl border border-border bg-card p-5 opacity-90">
            ...
            <span className="mt-3 inline-block rounded-full bg-muted px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Em breve</span>
          </div>
        ))}
      </div>
    </div>
